...
def get_exceedance_probability_curve(sim_losses_per_pt, plot = True):
    # Jovana sim_utils
    total_sim_losses_per_sim = sim_losses_per_pt.sum(axis=1).A1
    sorted_values = np.sort(total_sim_losses_per_sim)
...
def get_return_period_curve(sim_losses_per_pt, plot = True):
    at_event = sim_losses_per_pt.sum(axis=1).A1
    sorted_values = np.sort(at_event)

    # Calculate Return Periods
    n = sorted_values.size
    exceed_freq = np.linspace(1/n, 1, n)
    return_period = 1/exceed_freq[::-1]
    max_idx = int((1-1/250)*n+1)
...
def risk_measures(sim_losses_per_pt, printing = True):
    losses = sim_losses_per_pt.sum(1).A1
